...
需求见 `REQUIREMENTS.md`（v0.5）。当前：**25 校 29 院系 4174 人**——上交计算机学院 289 / AI 研究院 1（官网仅此 1 人）、清华 134、北大 119、哈工大 240（纯名录，官网无教师链接）、中科院计算所 283 / 自动化所 345 / 软件所 182 / 信工所 230（名单文章，官网无详情页）、中科大 72、南大 102、武大 109、同济 156、山大 17、中大 112、东北大学 147、吉大 185、华东师大 70、西工大 23（roster_only，教师系统被网络阻断）、人大高瓴 AI 学院 31、西电 155、中南 108、北师大 AI 学院 72、西交 75、川大 166、浙大 438（含 374 名录级导师，博导/硕导来自官方 2026 目录 xlsx，覆盖 99%）、**南科大 39、上科大 149、东南 125**（2026-09-05 新增）。字段含姓名/职称/博导硕导标识/研究所/邮箱/个人主页/研究方向/简介原文，全部带出处；**邮箱为套磁关键字段**（全库 2514/4174，低覆盖院系均核实为官网侧原因，见 HANDOVER §10.27）；方向 facet（topic.llm/cv/systems/... 13 类，`scripts/tag_facets.py` 可重算），列表页可按方向筛选。

站点范式：AJAX HTML（上交）、JSON API（上交 AI 研究院）、CAS 网站群（计算所/自动化所，含计算所 JS 变量内嵌人才库）、WP/博达 CMS（清华/中科大/南大/武大/同济/山大/北大/东北大学/吉大/华东师大/人大/西电，**meta description 兜底解析基本字段**）、JS 渲染名录（哈工大 tbody 网格、中大懒加载卡片、浙大名录）、tsites 教师主页系统（中南/西电/川大/西交——邮箱/电话密文可调官网 `tsitesencrypt.jsp` 服务端解密还原，见 `sites/tsites.py: decrypt_encrypted_fields`）、sudy WP generalQuery POST 接口（上科大）、官方 xlsx 导师目录（浙大，openpyxl）、纯文本名单页（东南 h2+ry-md）。Playwright 渲染已接入（缓存/滚动/networkidle 降级）。`roster_only` 配置开关用于详情页不可抓的站点（nwpu）。邮箱全库经混淆还原与校验（`crawler/email_util.py`）。
...
待处理（按处理方式分类）：
- **复旦**：唯一未接入 T1 校，名录在登录墙后
- **待定位名录页**：厦大、大工软件学院（ss.dlut.edu.cn）
- **华东师大补全**：名录按职称分类按钮加载，默认分类已抓 70 人，其余类别需 Playwright 逐类点击
- **网络不可达**：哈工大教师系统（faculty.hit.edu.cn，直连重置/代理 503）、北邮（scs.bupt.edu.cn）、重大（412）、湖大、电子科大、人大信息学院、华南理工（www2）、南开/华科（间歇）
- **浙大补全**：名录 64 人 vs xlsx 418 人，需过瑞数反爬
